...
export class RollTagEntityConverter {
  ...
  /**
   * Get character ID from an entity (parent_type and parent_id)
   * @param {string} parentType - Parent type (RollTagParentType constant)
   * @param {number} parentId - Parent entity ID
   * @param {string} guildId - Guild ID for database access
   * @returns {number|null} Character ID if the tag belongs to a character, null otherwise
   */
  static getCharacterIdFromEntity(parentType, parentId, guildId) {
    const db = getDbForGuild(guildId);
    
    // Query the parent entity to get character_id based on parent_type
    switch (parentType) {
      case RollTagParentType.CHARACTER_THEME: {
        const stmt = db.prepare('SELECT character_id FROM character_themes WHERE id = ?');
      ...
      case RollTagParentType.CHARACTER_BACKPACK: {
        const stmt = db.prepare('SELECT character_id FROM character_backpack WHERE id = ?');
        const result = stmt.get(parentId);
        return result ? result.character_id : null;
      }
      
      case RollTagParentType.CHARACTER_STORY_TAG: {
        const stmt = db.prepare('SELECT character_id FROM character_story_tags WHERE id = ?');
        const result = stmt.get(parentId);
        return result ? result.character_id : null;
      }
      
      case RollTagParentType.CHARACTER_STATUS: {
        const stmt = db.prepare('SELECT character_id FROM character_statuses WHERE id = ?');
        const result = stmt.get(parentId);
        return result ? result.character_id : null;
      }
      
      // Scene tags, fellowship tags don't have character_id
      case RollTagParentType.SCENE_TAG:
      case RollTagParentType.SCENE_STATUS:
      case RollTagParentType.FELLOWSHIP_TAG:
        return null;
  ...
  /**
   * Get character ID from a tag string by querying the parent entity
   * @param {string} tagString - Tag string like "theme:Tinkerer", "tag:SomeTag", etc.
   * @param {number} currentCharacterId - Current character ID (for character-related tags)
   * @param {string} sceneId - Scene ID (for scene-related tags)
   * @param {string} guildId - Guild ID for database access
   * @returns {number|null} Character ID if the tag belongs to a character, null otherwise
   */
  static getCharacterIdFromTag(tagString, currentCharacterId, sceneId, guildId) {
    // Parse the tag to get parent info
    const entityInfo = this.parseTagString(tagString, currentCharacterId, sceneId, guildId);
    if (!entityInfo) {
      return null;
    }
    
    return this.getCharacterIdFromEntity(entityInfo.parent_type, entityInfo.parent_id, guildId);
  }
}
